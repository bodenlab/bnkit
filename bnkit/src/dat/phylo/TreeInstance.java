...
/**
 * Class for representing the barebones of a tree (as defined by Tree via the TreeTopology interface) and
...
public class TreeInstance {
    ...
    private final TreeTopology tree;
    private final Map<Integer, Object> instantiation;
    private final Object[] possible;

    public TreeInstance(TreeTopology tree, Map<Integer, Object> values) {
        this.tree = tree;
        this.instantiation = values;
        Set s = new HashSet(instantiation.values());
        this.possible = new Object[s.size()];
        s.toArray(this.possible);
    }

    private Object getValueByIndex(int idx4val) {
        return possible[idx4val];
    }

    private int getIndexByValue(Object val) {
    ...
    public String toString(int bpidx) {
        ...
        Object instance = instantiation.get(bpidx);
        String istr = instance == null ? "?" : (LABEL_INCLUDES_INDEX ? "#" + bpidx + "_" + instance.toString() : instance.toString());
    ...
    /**
     * Inner class for inferring values on un-instantiated internal branchpoints
     */
    public class Parsimony {
        ...
        private final List[] optimal;           // [node idx] list of (optimal) values at branch point
        private final int nsym, nnodes;

        public Parsimony() {
            nnodes = tree.getSize();
            nsym = possible.length;
            scores = new double[nnodes][nsym];
            traceback = new int[nnodes][nsym][][];
            optimal = new List[nnodes];
            for (Map.Entry<Integer, Object> entry : instantiation.entrySet()) {
                int bpidx = entry.getKey();
                Object bpval = entry.getValue();
                optimal[bpidx] = Collections.singletonList(bpval);
                for (int i = 0; i < nsym; i++)
                    scores[bpidx][i] = bpval.equals(possible[i]) ? 0 : (double)Integer.MAX_VALUE;
            }
        }
        ...
        public void backward() {
            ...
            for (int parent_state : range(nsym, SET_RANDOM_PARSIMONY)) {
                if (scores[bpidx][parent_state] == scores[bpidx][best_index]) {
                    ...
                    optimal[bpidx].add(getValueByIndex(parent_state));
                    int[] children = tree.getChildren(bpidx);
                    for (int c = 0; c < children.length; c ++) {
        ...
        private void backward(int bpidx, int optimal_state) {
            ...
            if (optimal[bpidx].contains(getValueByIndex(optimal_state))) // check so that the state is assigned only once...
                return;                                 // ...avoid recurse since this value has been seen here before
            optimal[bpidx].add(getValueByIndex(optimal_state));
            int[] children = tree.getChildren(bpidx);
            for (int c = 0; c < children.length; c ++) {
                int childidx = children[c];
        ...
        public double getScore(int parent_value_index) {
            Object instance = instantiation.get(0);
            if (instance != null ) // this node is set
                return (parent_value_index == getIndexByValue(instance) ? 0 : Double.POSITIVE_INFINITY);
            double score = 0;
        ...
        private double getScore(int bpidx, int parent_value_index) {
            Object instance = instantiation.get(bpidx);
            if (instance != null ) { // this node is set
                return (parent_value_index == getIndexByValue(instance) ? 0 : 1);
        ...
        public String toString(int bpidx) {
            ...
            String istr = (LABEL_INCLUDES_INDEX ? "#" + bpidx + "_" : "?");
            if (assigns != null) {
                StringBuilder sb2 = (LABEL_INCLUDES_INDEX ? new StringBuilder("#" + bpidx + "_") : new StringBuilder());
